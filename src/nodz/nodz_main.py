...
class Nodz(QtWidgets.QGraphicsView):
    ...
    def mousePressEvent(self, event):
        ...
        if (event.button() == QtCore.Qt.LeftButton and QtCore.Qt.Key_Space in self.pressedKeys):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_G:
            print 'creating group nodel'
            self.createGroup('group node')
    ...
    def initialize(self):
        ...
        # Setup scene.
        self.sc = NodeScene(self)
        sceneWidth = config['scene_width']
        sceneHeight = config['scene_height']
        self.sc.setSceneRect(sceneWidth/2, sceneHeight/2, sceneWidth, sceneHeight)
        self.setScene(self.sc)
        # Connect scene node moved signal
        self.sc.signal_NodeMoved.connect(self.signal_NodeMoved)
    ...
    def createGroup(self, name='group', alternate=True, preset='node_default', **kwargs):
        """

        :param name:
        :return:
        """
        from node_objects import GroupNode
        nodes = self.scene().selectedItems()
        nodeItem = GroupItem(name=name, nodes=nodes, alternate=alternate, preset=preset,
    ...
    def createNode(self, name='default', preset='node_default', position=None, alternate=True, **kwargs):
        ...
        SocketAdd(parent=nodeItem, attribute='addSocket', index=-1, preset='attr_preset_1',
                  dataType=str, maxConnections=1)
        PlugAdd(parent=nodeItem, attribute='addPlug', index=-1, preset='attr_preset_1', dataType=str,
                maxConnections=1)
    ...
    def addDefaultSockets(self, nodeItem):
        pass
        # add default sockets
    ...
    def editNode(self, node, newName=None, **kwargs):
        """
    ...
    def saveGraph(self, filePath='path'):
        ...
        for node in nodes:
            ...
            data['NODES'][node] = {'preset': preset,
                                   ...
                                   'attributes': [],
                                   'preflight_data': nodeInst.preflight_data,
                                   'software': nodeInst.software,
                                   'description': nodeInst.description,
                                   'pipeID': nodeInst.pipeID}
    ...
    def loadGraph(self, filePath='path'):
        ...
        for name in nodesName:
            ...
            software = nodesData[name]['software']
            description = nodesData[name]['description']
            pipeID = nodesData[name]['pipeID']
            preflight_data = nodesData[name]['preflight_data']
...
class NodeItem(QtWidgets.QGraphicsItem):
    ...
    def create_preflight(self, name, description='', required=True, module=None):
        """
        Create a prefligth attribute on the top of the node info.
        :param name:
        :param description:
        :param module:
        :param required:
        ...
        # store the preflight data
        self.preflight_data[name] = {'description': description,
                                     'required': required,
                                     'module': module,
                                     'name': name}
    ...
    def paint(self, painter, option, widget):
        ...
        for attr in self.attrs:
            ...
            # Attribute rect. Builds the Square of the node
...
class SlotItem(QtWidgets.QGraphicsItem):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            for item in self.scene().items(mbb):
            ...
            newEndpoint = self.add_attr(nodzInst, self)

            if not self == target:
                if target.attribute == "addSocket" or target.attribute == "addPlug":
                    newStartpoint = self.add_attr(nodzInst, target)
                else:
                    newStartpoint = target

            # print "target attribute " + target.attribute
            # print "self attr: " + self.attribute
            ...
            if target.accepts(self):
                ...
                if newStartpoint and newEndpoint:
                    self.newConnection.nodInst = nodzInst
                    self.newConnection.target = newStartpoint
                    self.newConnection.source = newEndpoint
                    self.newConnection.target_point = newStartpoint.center()
                    self.newConnection.source_point = newEndpoint.center()
                    newStartpoint.connect(newEndpoint, self.newConnection)
                    newEndpoint.connect(newStartpoint, self.newConnection)
                    self.newConnection.updatePath()
                    # print "Both"
                    return
                ...
                if newEndpoint:
                    ...
                    self.newConnection.source_point = newEndpoint.center()
                    newEndpoint.connect(newEndpoint, self.newConnection)
    ...
    @staticmethod
    def check_type(this_item):
        ck = {'Input': False, 'Output': False}
        if isinstance(this_item, PlugAdd):
            ck['Output'] = True
        if isinstance(this_item, SocketAdd):
            ck['Input'] = True
        return ck

    def add_attr(self, nodzInst, nodeSocket):
        if nodeSocket.attribute == 'addSocket':
            pass
        elif nodeSocket.attribute == 'addPlug':
            pass
        else:
...
class SocketAdd(SocketItem):
    def __init__(self, parent, attribute, index, preset, dataType, maxConnections):
        super(SocketAdd, self).__init__(parent, attribute, index, preset, dataType, maxConnections)
    ...
    def _createStyle(self, parent):
        ...
        self._pen.setColor(utils._convertDataToColor(config[self.preset]['adds']))
        self.brush = QtGui.QBrush()
        self.brush.setStyle(QtCore.Qt.SolidPattern)
        self.brush.setColor(utils._convertDataToColor(config[self.preset]['adds']))
    ...
    def boundingRect(self):
        ...
        width = height = self.parentItem().attrHeight / 2.0
        x = - width / 2.0
        y = (self.parentItem().baseHeight/4)
        rect = QtCore.QRectF(QtCore.QRect(x, y, width, height))
        return rect
...
class PlugAdd(PlugItem):
    def __init__(self, parent, attribute, index, preset, dataType, maxConnections):
        super(PlugAdd, self).__init__(parent, attribute, index, preset, dataType, maxConnections)
    ...
    def boundingRect(self):
        ...
        y = (self.parentItem().baseHeight - config['node_radius'] +
            (self.parentItem().attrHeight/4) +
             len(self.parentItem().attrs) * self.parentItem().attrHeight )
...
class ConnectionItem(QtWidgets.QGraphicsItemGroup):
    ...
    def _createStyle(self):
        ...
        self.manualColor = QtGui.QColor(155, 50, 50)
        self.autoColor = QtGui.QColor(50, 155, 50)
        self.penWidth = config['connection_width']

        self.defaultColor = utils._convertDataToColor(config['connection_color'])

        self._pen = QtGui.QPen(self.defaultColor)
        self._pen.setWidth(self.penWidth)

        self.autoPen = QtGui.QPen(self.autoColor)
        self.autoPen.setWidth(self.penWidth)

        self.manualPen = QtGui.QPen(self.manualColor)
        self.manualPen.setWidth(self.penWidth)
